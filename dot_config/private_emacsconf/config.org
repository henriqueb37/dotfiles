...
* BACKUP FILES
This should fix some annoyances with backup files created automatically by emacs.  All backup files will be placed in a backup directory in the config directory.

#+begin_src emacs-lisp
(setq backup-directory-alist `(("." . ,(file-name-concat user-emacs-directory "backup/"))))
...
(use-package hyperbole
  :diminish
  :init
  ;; Prevent hyperbole from opening Rust files as Raster image files
  (setq hpath:external-file-suffixes
        "e?ps\\|dvi\\|pdf\\|ps\\.g?[zZ]\\|gif\\|tiff?\\|xpm\\|xbm\\|xwd\\|pm\\|pbm\\|jpe?g\\|ras\\|xcf")
  :config
  (hyperbole-mode 1))
#+end_src

* LANGUAGE SUPPORT/LSP
Major mode and LSP support for Emacs.

This uses Emacs's builtin LSP support (eglot).

+ Eldoc box provides a nice frame that shows documentation on hover,
+ Sideline shows information on various things on the side.

#+begin_src emacs-lisp
(use-package eglot
  :elpaca nil
  :config
  (defvar my-lsps-directory
          (file-name-concat user-emacs-directory "lsps/")
          "The directory where lsp server programs should be placed.")
  (defun +eglot-add-server-program (major-mode server-path)
    "Add MAJOR-MODE to `eglot-server-programs' where SERVER-PATH is relative to `my-lsps-directory'"
    (add-to-list 'eglot-server-programs
                 `(,major-mode . (,(file-name-concat
                                    my-lsps-directory
                                    server-path))))))
...
(use-package rust-mode
  ...
  :init
  ;; Doesn't seem to load otherwise???????????
  ;; I have no idea why
  (require 'rust-mode)
  :config
  (add-to-list
...
(use-package magit
  ...
  :config
  (my/leader-keys
    "c g" '(magit-status :wk "Git status")))
...
Org mode source blocks have some really weird and annoying default indentation behavior.  The problems seem to have been fixed by just setting =org-src-preserve-indentation=.
